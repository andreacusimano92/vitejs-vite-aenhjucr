...
const formatSimpleDate = (seconds) => {
  if (!seconds) return '-';
      ...
      <div className="bg-white w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="p-8 space-y-6">
          <div className="flex justify-between items-start">
             <div>
               <h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter">Dettaglio Lavoro</h3>
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{report.reportDate}</p>
             </div>
             {canDelete && <button onClick={handleDelete} className="p-3 text-red-500 bg-red-50 rounded-2xl"><Trash2 size={20}/></button>}
          </div>
          <div className="grid grid-cols-2 gap-4">
             <div className="p-4 bg-slate-50 rounded-3xl"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Ore</p><p className="font-bold text-slate-700">{report.hours} h</p></div>
             <div className="p-4 bg-slate-50 rounded-3xl"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Mezzo</p><p className="font-bold text-slate-700 truncate text-xs">{report.vehicleName}</p></div>
          </div>
          <div className="p-5 bg-slate-50 rounded-3xl border">
             <p className="text-[9px] font-black text-slate-400 uppercase mb-2">Descrizione</p>
             <p className="text-sm text-slate-600 leading-relaxed">{report.desc}</p>
          </div>
          <div className="grid grid-cols-2 gap-4">
             {report.fuelAmount && <div className="p-4 bg-green-50 rounded-3xl text-green-700"><p className="text-[9px] font-black uppercase mb-1">Carburante</p><p className="font-bold">€ {report.fuelAmount}</p></div>}
             {report.equipmentUsed && <div className="p-4 bg-blue-50 rounded-3xl text-blue-700"><p className="text-[9px] font-black uppercase mb-1">Attrezzatura</p><p className="font-bold text-xs">{report.equipmentUsed}</p></div>}
          </div>
          <button onClick={onClose} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-xs tracking-widest">Chiudi</button>
        </div>
      </div>
    </div>
  );
...
function SiteReportsList({ taskId, isMaster, userData, canDelete }) {
  ...
  return (
    <div className="space-y-4">
      {reports.length === 0 && <p className="text-center py-20 text-slate-300 font-black uppercase text-xs">Nessun dato storico</p>}
      {reports.map(r => (
        <div key={r.id} onClick={()=>setSelected(r)} className="p-6 bg-white border rounded-[36px] flex justify-between items-center shadow-sm cursor-pointer hover:border-blue-300 transition-all">
          <div className="flex-1 min-w-0">
             <div className="flex items-center gap-2">
                <span className="text-[10px] font-black text-slate-400 uppercase">{r.reportDate}</span>
          ...
          <div className="text-right ml-4"><p className="text-xl font-black text-slate-800">{r.hours}h</p></div>
        </div>
      ))}
      {selected && <ReportModal report={selected} onClose={()=>setSelected(null)} canDelete={canDelete} />}
    </div>
  );
}
...
function SiteOverview({ task, isMaster, isAdminFull, userData }) {
    ...
    <div className="space-y-6">
      <div className={`p-6 rounded-[32px] border flex justify-between items-center ${task.completed ? 'bg-slate-900 border-slate-800 text-white' : 'bg-white border-slate-200'}`}>
        <div className="flex items-center gap-4">
          ...
          <div><h4 className="font-black uppercase text-[10px]">Stato</h4><p className="text-sm font-bold">{task.completed ? 'CHIUSO' : 'OPERATIVO'}</p></div>
        </div>
        {isMaster && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-3xl border text-center shadow-sm"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Forniture</p><p className="text-xl font-black text-slate-800">€ {stats.mat.toFixed(0)}</p></div>
        ...
        <div className="bg-slate-50 p-5 rounded-3xl border text-center"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Prossima Fase</p><p className="text-xs font-bold truncate uppercase">{next ? next.name : 'Nessuna'}</p></div>
      </div>
    </div>
  );
}
...
function SiteTeam({ task, isAdminFull }) {
  ...
  const handleAssign = async () => {
    if(!selectedUser || !isAdminFull || task.completed) return;
        ...
        {assigned.map(name => {
          ...
          return (
            <div key={name} className="p-4 bg-white border rounded-[28px] flex justify-between items-center shadow-sm border-slate-100">
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center font-black text-sm ${staff?.role === 'Master' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>{name.charAt(0)}</div>
                <span className="text-sm font-black text-slate-800 uppercase tracking-tighter">{name}</span>
              </div>
              {isAdminFull && !task.completed && <button onClick={async () => await updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'tasks', task.id), { assignedTeam: arrayRemove(name) })} className="text-red-400 hover:bg-red-50 p-2 rounded-xl transition-colors"><X size={18}/></button>}
            </div>
          );
        })}
      </div>
...
// ... ALTRE SEZIONI (CHAT, DOCS, SCHEDULE, PHOTOS, REQUESTS) ...
...
function SiteChat({ taskId, userData, isClosed }) {
  ...
  return (
    <div className="flex flex-col h-[450px] bg-white rounded-[40px] border overflow-hidden shadow-sm">
      ...
      {!isClosed ? <form onSubmit={sendMessage} className="p-4 bg-white border-t flex gap-2"><input className="flex-1 bg-slate-100 border-none rounded-2xl px-4 py-2 text-sm outline-none font-bold" placeholder="Messaggio..." value={newMessage} onChange={e=>setNewMessage(e.target.value)} /><button type="submit" className="bg-blue-600 text-white p-3 rounded-2xl active:scale-90 transition-transform"><Send size={20}/></button></form> : <div className="p-4 text-center text-[10px] font-black uppercase text-slate-400 border-t">Cantiere Chiuso</div>}
    </div>
  );
}
...
function PersonalAreaView({ user, userData, isMaster, isAdminFull }) {
  ...
  return (
    <div className="space-y-6">
       ...
       {sub === 'logs' && isMaster && (
          <div className="bg-white border rounded-[32px] overflow-hidden shadow-sm overflow-x-auto animate-in fade-in">
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-50 text-slate-400 font-black uppercase tracking-widest border-b"><tr><th className="p-5">Nome</th><th className="p-5">Azione</th><th className="p-5">Data</th></tr></thead>
                <tbody className="divide-y">{logs.map(l=>(<tr key={l.id} className="hover:bg-slate-50"><td className="p-5 font-bold uppercase text-[10px]">{l.userName}</td><td className="p-5">{l.action}</td><td className="p-5 text-[9px] text-slate-400">{formatSimpleDate(l.createdAt?.seconds)}</td></tr>))}</tbody>
              </table>
          </div>
       )}
    </div>
        ...
        {leaves.map(req => (
            <div key={req.id} className="bg-white p-6 rounded-[32px] border shadow-sm flex justify-between items-center border-slate-100">
              ...
              {isAdmin && targetIdentifier !== currentUser.uid && req.status === 'pending' && (
                <div className="flex gap-2"><button onClick={() => handleStatus(req.id, 'approved')} className="bg-green-600 text-white px-5 py-2 rounded-xl text-xs font-black uppercase tracking-widest shadow-md shadow-green-100 transition-all hover:scale-105">Ok</button><button onClick={() => handleStatus(req.id, 'rejected')} className="bg-red-50 text-red-600 px-5 py-2 rounded-xl text-xs font-black uppercase border border-red-100">No</button></div>
              )}
            </div>
          ))}
...
function TaskDetailContainer({ task, userData, isMaster, isAdminFull, onBack }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <button onClick={onBack} className="flex items-center gap-2 text-slate-400 text-sm font-bold uppercase tracking-widest hover:text-blue-600 transition-colors"><ArrowLeft size={16}/> Lista Cantieri</button>
      ...
      <div className="mt-4">
        ...
        {active === 'reports' && <SiteReportsList taskId={task.id} isMaster={isMaster} userData={userData} canDelete={isAdminFull} />}
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
         ...
         {isAdminFull && !showForm && <button onClick={()=>setShowForm(true)} className="bg-blue-600 text-white px-6 py-3 rounded-2xl shadow-xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-transform"><Plus size={18}/> Registra Nuovo</button>}
      </div>
      {showForm && (
        <form onSubmit={add} className="bg-white p-10 rounded-[40px] border shadow-2xl space-y-5 animate-in slide-in-from-top-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input placeholder="Titolo" className="bg-slate-50 border-none rounded-2xl p-5 outline-none font-bold" onChange={e=>setForm({...form, title: e.target.value})} required />
            <input placeholder="Cliente" className="bg-slate-50 border-none rounded-2xl p-5 outline-none font-bold" onChange={e=>setForm({...form, client: e.target.value})} required />
          </div>
          ...
          <div className="flex gap-4 pt-4"><button type="submit" className="bg-blue-600 text-white px-12 py-4 rounded-3xl font-black text-xs uppercase tracking-widest shadow-xl">Salva Dati</button><button type="button" onClick={()=>setShowForm(false)} className="text-slate-400 font-bold uppercase text-xs px-6">Annulla</button></div>
        </form>
      )}
    ...
    <div className="space-y-8 max-w-2xl mx-auto">
      <form onSubmit={submit} className="bg-white p-10 rounded-[48px] border shadow-xl space-y-5 animate-in slide-in-from-bottom-4">
        ...
        <div className="grid grid-cols-2 gap-4">
           ...
           <select value={form.vehicleId} onChange={e=>setForm({...form, vehicleId: e.target.value})} className="flex-1 bg-slate-50 border-none rounded-2xl p-4 font-bold text-sm shadow-inner" required>{STATIC_VEHICLES.map(v => <option key={v} value={v}>{v}</option>)}</select>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <button type="submit" className="w-full bg-blue-600 text-white py-5 rounded-[28px] font-black uppercase tracking-widest text-sm shadow-xl">Invia</button>
      </form>
      <div className="space-y-4">
        {reports.map(r => (
          <div key={r.id} onClick={()=>setSelected(r)} className="p-6 bg-white border rounded-[36px] flex justify-between items-center shadow-sm border-slate-100 cursor-pointer hover:bg-slate-50 transition-all">
            <div className="flex-1 pr-4"><p className="font-bold text-slate-800 line-clamp-1 uppercase text-sm">{r.desc}</p><p className="text-[9px] text-slate-400 mt-2 uppercase font-black tracking-widest">{r.userName} • {r.taskTitle} • {r.reportDate} • {r.hours}h {r.isTrasferta ? '• TRASFERTA' : ''}</p></div>
          </div>
        ))}
