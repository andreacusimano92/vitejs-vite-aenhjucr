...
function MaterialRequestsView({ taskId, user, userData, isMaster, task }) {
  ...
  const deleteRequest = async (id) => {
    if (!isMaster && !window.confirm("Cancellare richiesta?")) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        {loading ? <Loader2 className="w-6 h-6 animate-spin mx-auto text-blue-600 my-4" /> : requests.length === 0 ? 
          <p className="text-center text-slate-400 text-sm py-8">Nessuna richiesta di materiale attiva.</p> :
          requests.map(req => (
