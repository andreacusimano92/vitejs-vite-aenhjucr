...
const formatTimestamp = (ts) => {
  ...
  return new Date(ts.seconds * 1000).toLocaleString('it-IT', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' });
};
...
// --- COMPONENTI UI FONDAMENTALI ---

function LoadingScreen() {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 text-slate-400">
      <Loader2 className="w-10 h-10 animate-spin text-blue-600 mb-4" />
      <p className="text-xs font-black uppercase tracking-widest animate-pulse">Sincronizzazione Sistema...</p>
    </div>
  );
}
...
function ReportModal({ report, onClose, canDelete }) {
  ...
  const handleDelete = async () => {
    if (!window.confirm("Eliminare questo rapporto?")) return;
    try {
...
function SiteOverview({ task, isMaster, isAdminFull, userData }) {
  ...
  useEffect(() => {
    // Semplificazione: Carichiamo tutto e filtriamo in locale per evitare crash indici
    const unsubM = onSnapshot(collection(db, 'artifacts', APP_ID, 'public', 'data', 'materials'), s => {
      const filtered = s.docs.map(d=>d.data()).filter(m => m.taskId === task.id);
      setStats(prev => ({...prev, mat: filtered.reduce((sum, d) => sum + (parseFloat(d.quantity || 0) * parseFloat(d.cost || 0)), 0)}));
    });
    const unsubL = onSnapshot(collection(db, 'artifacts', APP_ID, 'public', 'data', 'daily_reports'), s => {
      const filtered = s.docs.map(d=>d.data()).filter(r => r.taskId === task.id);
      setStats(prev => ({...prev, hrs: filtered.reduce((sum, d) => sum + (parseFloat(d.hours || 0)), 0), travel: filtered.filter(d=>d.isTrasferta).length}));
    });
    ...
    <div className="space-y-6 animate-in fade-in">
      <div className={`p-6 rounded-[32px] border flex justify-between items-center ${task.completed ? 'bg-slate-900 border-slate-800 text-white shadow-xl' : 'bg-white border-slate-200 shadow-sm'}`}>
        <div className="flex items-center gap-4">
          ...
          <div><h4 className="font-black uppercase text-[10px] tracking-widest">Stato Cantiere</h4><p className="text-sm font-bold">{task.completed ? 'CHIUSO / ARCHIVIATO' : 'CANTIERE OPERATIVO'}</p></div>
        </div>
        {isMaster && (
          ...
          }} className={`px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg ${task.completed ? 'bg-blue-600' : 'bg-red-600'} text-white`}>
...
function SiteReportsList({ taskId, isMaster, userData, canDelete }) {
  ...
  useEffect(() => {
    return onSnapshot(collection(db, 'artifacts', APP_ID, 'public', 'data', 'daily_reports'), (snap) => {
...
function SiteChat({ taskId, userData, isClosed }) {
  ...
  useEffect(() => {
    return onSnapshot(collection(db, 'artifacts', APP_ID, 'public', 'data', 'site_chats'), (snap) => {
      const filtered = snap.docs.map(d => ({id: d.id, ...d.data()})).filter(m => m.taskId === taskId).sort((a,b) => (a.createdAt?.seconds || 0) - (b.createdAt?.seconds || 0));
...
function LeaveRequestsPanel({ currentUser, targetIdentifier, isAdmin, userData }) {
  ...
  useEffect(() => {
    return onSnapshot(collection(db, 'artifacts', APP_ID, 'public', 'data', 'leaves'), (snap) => {
      const all = snap.docs.map(d => ({id: d.id, ...d.data()}));
      ...
      {targetIdentifier === currentUser.uid && (
        <div className="bg-white p-8 rounded-[40px] border shadow-sm h-fit space-y-4">
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-blue-600 text-white p-4 rounded-2xl font-black uppercase text-xs shadow-lg">Invia</button>
          </form>
        </div>
      )}
            ...
            return (
              <div key={req.id} className="bg-white p-6 rounded-[32px] border shadow-sm flex justify-between items-center border-slate-100">
                ...
                {isAdmin && targetIdentifier !== currentUser.uid && req.status === 'pending' && (
                  <div className="flex gap-2"><button onClick={() => updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'leaves', req.id), {status:'approved'})} className="bg-green-600 text-white px-5 py-2 rounded-xl text-xs font-black uppercase">Sì</button><button onClick={() => updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'leaves', req.id), {status:'rejected'})} className="bg-red-50 text-red-600 px-5 py-2 rounded-xl text-xs font-black uppercase border">No</button></div>
                )}
              </div>
            );
          })}
...
function PersonalAreaView({ user, userData, isMaster, isAdminFull }) {
  ...
  useEffect(() => {
    if(isMaster) { 
      ...
      onSnapshot(collection(db, 'artifacts', APP_ID, 'public', 'data', 'audit_logs'), s => {
        const all = s.docs.map(d=>({id:d.id, ...d.data()})).sort((a,b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0)).slice(0, 30);
        setLogs(all);
      });
    }
  }, [isMaster]);
...
function MaterialsView() {
  ...
  useEffect(() => {
    return onSnapshot(collection(db, 'artifacts', APP_ID, 'public', 'data', 'materials'), s => {
...
function DashboardContainer({ user, userData }) {
  ...
  useEffect(() => {
    ...
    if (!user) return;
    return onSnapshot(collection(db, 'artifacts', APP_ID, 'public', 'data', 'notifications'), (snap) => {
      const all = snap.docs.map(d => ({ id: d.id, ...d.data() }))
        .filter(n => n.targetUserId === 'all' || (n.targetUserId === 'all_masters' && isMaster) || n.targetUserId === user.uid)
        .sort((a,b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));
