        ...
        <div className="p-8 space-y-6">
          <div className="flex justify-between items-start border-b pb-4">
             ...
             <div className="flex gap-2">
                {canDelete && <button onClick={handleDelete} className="p-3 text-red-500 bg-red-50 rounded-2xl hover:bg-red-100 transition-colors"><Trash2 size={20}/></button>}
          ...
          <div className="grid grid-cols-2 gap-4">
             <div className="p-4 bg-slate-50 rounded-3xl">
                <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Operatore</p>
                <p className="font-bold text-slate-700 text-sm">{report.userName}</p>
             </div>
             <div className="p-4 bg-slate-50 rounded-3xl">
                <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Ore Totali</p>
                <p className="font-bold text-blue-600 text-xl">{report.hours} h</p>
             </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
             <div className="p-4 bg-slate-50 rounded-3xl border">
                <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Veicolo</p>
                <div className="flex items-center gap-2 text-slate-700 font-bold text-[10px] truncate"><Truck size={14}/> {report.vehicleName}</div>
             </div>
             <div className="p-4 bg-slate-50 rounded-3xl border">
                <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Carburante</p>
                <div className="flex items-center gap-2 text-green-600 font-bold text-[10px]"><Fuel size={14}/> {report.fuelAmount ? `€ ${report.fuelAmount}` : 'No'}</div>
             </div>
          </div>
          {report.equipmentUsed && (
            <div className="p-4 bg-blue-50/50 rounded-3xl border border-blue-100">
              ...
              <div className="flex items-center gap-2 text-blue-700 font-bold text-[10px]"><Construction size={14}/> {report.equipmentUsed}</div>
            </div>
          )}
        </div>
      </div>
      ...
      {reports.map(r => (
        <div key={r.id} onClick={()=>setSelected(r)} className="p-6 bg-white border rounded-[36px] flex justify-between items-center shadow-sm cursor-pointer hover:border-blue-300 transition-all border-slate-100 group">
          ...
          <div className="text-right">
             <p className="text-2xl font-black text-slate-800 leading-none">{r.hours}<span className="text-xs">h</span></p>
          </div>
        </div>
      ))}
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
         ...
         {isAdminFull && !showForm && <button onClick={()=>setShowForm(true)} className="bg-blue-600 text-white px-6 py-3 rounded-2xl shadow-xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-transform"><Plus size={18}/> Nuovo</button>}
      </div>
      {showForm && (
        <form onSubmit={add} className="bg-white p-10 rounded-[40px] border shadow-2xl space-y-5 animate-in slide-in-from-top-4">
          ...
          <div className="flex gap-4"><button type="submit" className="bg-blue-600 text-white px-12 py-4 rounded-3xl font-black text-xs uppercase tracking-widest shadow-xl">Salva</button><button type="button" onClick={()=>setShowForm(false)} className="px-4 text-slate-400 font-bold uppercase text-xs">Annulla</button></div>
        </form>
      )}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {tasks.map(t => (
          <div key={t.id} onClick={()=>onSelectTask(t)} className="bg-white p-7 border rounded-[48px] hover:ring-4 hover:ring-blue-600/10 cursor-pointer shadow-sm relative overflow-hidden group h-48 flex flex-col justify-between">
    ...
    <div className="space-y-8 max-w-2xl mx-auto">
      <form onSubmit={submit} className="bg-white p-10 rounded-[48px] border shadow-xl space-y-6 animate-in slide-in-from-bottom-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <select value={form.taskId} onChange={e=>setForm({...form, taskId: e.target.value})} className="bg-slate-50 border-none rounded-3xl p-5 outline-none font-bold text-sm shadow-inner" required><option value="">Scegli Cantiere...</option>{tasks.filter(t=>!t.completed).map(t=><option key={t.id} value={t.id}>{t.title}</option>)}</select>
          <input type="date" value={form.reportDate} onChange={e=>setForm({...form, reportDate: e.target.value})} className="bg-slate-50 border-none rounded-3xl p-5 outline-none font-bold text-sm shadow-inner" required />
        </div>
        <div className="grid grid-cols-2 gap-4">
        ...
        <div className="flex items-center justify-between p-4 bg-blue-50 rounded-3xl border border-blue-100">
           ...
           <button type="button" onClick={()=>setForm({...form, isTrasferta: !form.isTrasferta})} className={`w-14 h-7 rounded-full transition-colors relative ${form.isTrasferta ? 'bg-blue-600' : 'bg-slate-300'}`}><div className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all ${form.isTrasferta ? 'left-8' : 'left-1'}`}></div></button>
        </div>
        <textarea placeholder="Cosa hai fatto?" value={form.desc} onChange={e=>setForm({...form, desc: e.target.value})} className="w-full bg-slate-50 border-none rounded-3xl p-5 font-bold text-sm shadow-inner outline-none" rows="4" required />
        ...
        {leaves.map(req => (
            <div key={req.id} className="bg-white p-6 rounded-[32px] border shadow-sm flex justify-between items-center border-slate-100">
              ...
              {isAdmin && targetIdentifier !== currentUser.uid && req.status === 'pending' && (
                <div className="flex gap-2"><button onClick={() => updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'leaves', req.id), {status:'approved'})} className="bg-green-600 text-white px-5 py-2 rounded-xl text-xs font-black uppercase shadow-lg transition-all hover:scale-105">Sì</button><button onClick={() => updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'leaves', req.id), {status:'rejected'})} className="bg-red-50 text-red-600 px-5 py-2 rounded-xl text-xs font-black uppercase border border-red-100 transition-all hover:bg-red-100">No</button></div>
              )}
            </div>
          ))}
      </div>
  ...
  return (
    <div className="space-y-6">
       <div className="bg-white p-8 rounded-[40px] border shadow-sm flex flex-col sm:flex-row justify-between items-center gap-6">
         ...
         <div className="flex gap-4 w-full sm:w-auto overflow-x-auto scrollbar-hide"><button onClick={()=>setSub('leaves')} className={`flex-1 sm:px-6 py-2 text-xs font-black uppercase transition-all ${sub === 'leaves' ? 'bg-blue-600 text-white rounded-2xl shadow-lg' : 'text-slate-400'}`}>Ferie</button>{isMaster && <button onClick={()=>setSub('logs')} className={`flex-1 sm:px-6 py-2 text-xs font-black uppercase transition-all ${sub === 'logs' ? 'bg-blue-600 text-white rounded-2xl shadow-lg' : 'text-slate-400'}`}>Log</button>}</div>
       </div>
