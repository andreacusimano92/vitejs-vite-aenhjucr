...
function LoadingScreen() {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 text-slate-400">
      ...
      <p className="text-xs font-black uppercase tracking-widest">Sincronizzazione in corso...</p>
    </div>
  );
}
...
function SiteOverview({ task, isMaster, isAdmin, userData }) {
  ...
  const handleToggleStatus = async () => {
    ...
    if (newStatus && !window.confirm("Sei sicuro di voler terminare il cantiere?")) return;
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className={`p-6 rounded-[32px] border flex flex-col sm:flex-row justify-between items-center gap-4 ${task.completed ? 'bg-slate-900 border-slate-800 text-white shadow-xl' : 'bg-white border-slate-200'}`}>
        <div className="flex items-center gap-4">
          ...
          <div>
            <h4 className="font-black uppercase tracking-tighter">Stato Cantiere</h4>
            <p className={`text-xs font-bold ${task.completed ? 'text-slate-400' : 'text-blue-500'}`}>{task.completed ? 'ARCHIVIATO' : 'OPERATIVO'}</p>
          </div>
        </div>
        {isMaster && (
          <div className="flex gap-2 w-full sm:w-auto">
            {!task.completed ? (
              <button onClick={handleToggleStatus} className="w-full sm:w-auto bg-red-600 text-white px-6 py-2.5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg">Chiudi Lavori</button>
            ) : isAdmin && (
              <button onClick={handleToggleStatus} className="w-full sm:w-auto bg-blue-600 text-white px-6 py-2.5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg">Riapri Cantiere</button>
            )}
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-3xl border shadow-sm"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Ore</p><p className="text-xl font-black text-slate-800">{totals.hours} H</p></div>
        <div className="bg-white p-5 rounded-3xl border shadow-sm"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Materiali</p><p className="text-xl font-black text-slate-800">€ {totals.materials.toFixed(0)}</p></div>
        ...
        <div className="bg-slate-50 p-5 rounded-3xl border border-slate-200"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Zona</p><p className="text-sm font-black text-slate-600 uppercase">{task.isTrasfertaSite ? 'Fuori Sede' : 'Locale'}</p></div>
      </div>
    </div>
  );
}
...
function SiteDocuments({ taskId, isAdmin, userData, isClosed }) {
  ...
  return (
    <div className="space-y-4">
      {isAdmin && !isClosed && <div className="bg-white p-5 rounded-[32px] border border-dashed border-slate-300 flex justify-between items-center"><p className="text-xs font-black text-slate-500 uppercase tracking-widest">Repository File</p><input type="file" onChange={handleUpload} className="text-xs" /></div>}
    ...
    <div className="space-y-4">
      {isAdmin && !task.completed && (
        <form onSubmit={addPhase} className="bg-white p-5 rounded-[32px] border grid grid-cols-1 md:grid-cols-4 gap-4 shadow-sm">
          ...
          <button type="submit" className="bg-blue-600 text-white rounded-2xl font-black text-xs uppercase shadow-lg">Salva</button>
        </form>
      )}
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-5 rounded-[32px] border shadow-sm">
        <h3 className="font-black text-slate-800 uppercase tracking-tighter">Album Foto</h3>
  ...
  return (
    <div className="space-y-8 max-w-2xl mx-auto">
      <form onSubmit={submit} className="bg-white p-10 rounded-[48px] border shadow-xl space-y-6">
        ...
        <button type="submit" className="w-full bg-blue-600 text-white py-5 rounded-[28px] font-black uppercase shadow-xl">Salva Report</button>
      </form>
    </div>
  );
}
    ...
    <div className="space-y-8">
      {isAdmin && (
        <form onSubmit={add} className="bg-white p-10 rounded-[48px] border shadow-lg space-y-6">
          ...
          <button type="submit" className="w-full bg-blue-600 text-white py-5 rounded-3xl font-black uppercase tracking-widest shadow-lg">Registra</button>
        </form>
      )}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {vehicles.map(v => {
        ...
        {leaves.map(req => (
            <div key={req.id} className="bg-white p-6 rounded-[32px] border shadow-sm flex justify-between items-center transition-all border-slate-100">
              ...
              {isAdmin && targetIdentifier !== currentUser.uid && req.status === 'pending' && (
                <div className="flex gap-2"><button onClick={() => handleStatus(req.id, 'approved')} className="bg-green-600 text-white px-5 py-2 rounded-xl text-xs font-black uppercase shadow-lg">Ok</button><button onClick={() => handleStatus(req.id, 'rejected')} className="bg-red-50 text-red-600 px-5 py-2 rounded-xl text-xs font-black uppercase border border-red-100">No</button></div>
              )}
            </div>
          ))}
      </div>
...
function TaskDetailContainer({ task, userData, isMaster, isAdmin, onBack }) {
  ...
  const tabs = [ 
    ...
    { id: 'materials', label: 'Logist', icon: Package }, 
