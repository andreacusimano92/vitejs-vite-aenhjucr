...
function LoadingScreen() {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 text-slate-500">
      ...
      <p className="font-medium animate-pulse text-sm uppercase tracking-widest">Sincronizzazione Sistema...</p>
    </div>
  );
}
...
function SiteOverview({ task, isMaster, isAdmin, userData }) {
  ...
  const toggleTravelSite = async () => {
    ...
    await updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'tasks', task.id), { 
      isTrasfertaSite: !task.isTrasfertaSite 
    });
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white p-6 rounded-[32px] border shadow-sm flex flex-col justify-between">
           <div className="flex justify-between items-start">
             <div>
               <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest">Località</h4>
             ...
             {isAdmin && !task.completed && (
               <button onClick={toggleTravelSite} className="text-[9px] font-black uppercase text-blue-600 bg-blue-50 px-3 py-1 rounded-lg">Cambia</button>
             )}
           </div>
           <div className="mt-4 pt-4 border-t flex items-center gap-4">
              ...
              <div>
                 ...
                 <p className="text-xl font-black text-slate-800">{totals.travel}</p>
              </div>
           </div>
        </div>
        <div className="bg-white p-6 rounded-[32px] border shadow-sm">
           <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Statistiche Lavoro</h4>
           <div className="grid grid-cols-2 gap-4">
              <div><p className="text-[10px] font-black text-slate-400 uppercase">Ore</p><p className="text-xl font-black text-slate-800">{totals.hours} H</p></div>
              <div><p className="text-[10px] font-black text-slate-400 uppercase">Materiali</p><p className="text-xl font-black text-slate-800">€ {totals.materials.toFixed(0)}</p></div>
           </div>
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-col h-[400px] bg-white rounded-3xl border overflow-hidden shadow-sm">
      ...
      {!isClosed ? (
        <form onSubmit={sendMessage} className="p-3 bg-white border-t flex gap-2">
          ...
          <button type="submit" className="bg-blue-600 text-white p-2 rounded-xl transition-all active:scale-90"><Send size={18}/></button>
        </form>
    ...
    <div className="space-y-4">
      {isAdmin && !task.completed && (
        <div className="flex gap-2">
          <select value={selectedUser} onChange={e=>setSelectedUser(e.target.value)} className="flex-1 border rounded-xl p-3 text-sm font-bold bg-white">
    ...
    <div className="space-y-4">
      {isAdmin && !task.completed && (
        <form onSubmit={addPhase} className="bg-white p-5 rounded-[32px] border grid grid-cols-1 md:grid-cols-4 gap-3 shadow-sm">
          <input placeholder="Fase" className="bg-slate-50 border-none rounded-2xl p-4 font-bold text-sm" value={newPhase.name} onChange={e=>setNewPhase({...newPhase, name: e.target.value})} />
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-5 rounded-[32px] border shadow-sm">
        <h3 className="font-black text-slate-800 uppercase tracking-tighter">Foto Cantiere</h3>
...
function SiteAccounting({ taskId }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div className="bg-white p-6 rounded-3xl border shadow-sm text-center"><p className="text-[9px] text-slate-400 uppercase font-black tracking-widest mb-1">Lavoro</p><p className="text-xl font-bold text-slate-800">€ {totals.labor.toFixed(0)}</p></div>
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
         ...
         {isAdmin && !isFormOpen && <button onClick={()=>setIsFormOpen(true)} className="bg-blue-600 text-white px-6 py-3 rounded-2xl shadow-xl font-black text-xs uppercase tracking-widest"><Plus size={18}/> Nuovo</button>}
      </div>
      {isFormOpen && (
        <form onSubmit={addTask} className="bg-white p-10 rounded-[40px] border shadow-2xl space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input placeholder="Titolo" className="bg-slate-50 rounded-2xl p-5 outline-none font-bold" onChange={e=>setNewTask({...newTask, title: e.target.value})} required />
            <input placeholder="Cliente" className="bg-slate-50 rounded-2xl p-5 outline-none font-bold" onChange={e=>setNewTask({...newTask, client: e.target.value})} required />
          </div>
  ...
  return (
    <div className="space-y-6">
       <div className="bg-white p-8 rounded-[40px] border shadow-sm flex flex-col sm:flex-row justify-between items-center gap-6">
         <div className="flex items-center gap-5 w-full sm:w-auto"><div className="w-16 h-16 bg-blue-600 rounded-3xl flex items-center justify-center text-white text-2xl font-black shadow-xl uppercase">{userData?.name?.charAt(0)}</div><div><h2 className="text-2xl font-black text-slate-800 tracking-tighter uppercase leading-none">{userData?.name}</h2>{isMaster && <select className="mt-3 text-[10px] font-black uppercase tracking-widest text-blue-600 bg-blue-50 border-none rounded-xl p-2 outline-none" value={targetUser} onChange={(e) => setTargetUser(e.target.value)}><option value={user.uid}>Mio Profilo</option>{usersList.map(u => (<option key={u.username} value={u.username}>Vedi: {u.name}</option>))}</select>}</div></div>
         <div className="flex gap-4 w-full sm:w-auto"><button onClick={()=>setSub('leaves')} className={`flex-1 sm:px-6 py-2 text-xs font-black uppercase transition-all ${sub === 'leaves' ? 'bg-blue-600 text-white rounded-2xl shadow-lg' : 'text-slate-400'}`}>Ferie</button><button onClick={()=>setSub('logs')} className={`flex-1 sm:px-6 py-2 text-xs font-black uppercase transition-all ${sub === 'logs' ? 'bg-blue-600 text-white rounded-2xl shadow-lg' : 'text-slate-400'}`}>Logs</button></div>
       </div>
...
function TaskDetailContainer({ task, userData, isMaster, isAdmin, onBack }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <button onClick={onBack} className="flex items-center gap-2 text-slate-400 text-sm font-bold uppercase tracking-widest hover:text-blue-600"><ArrowLeft size={16}/> Indietro</button>
      <div className="bg-white p-6 rounded-[32px] border shadow-sm">
