...
import { 
  ...
  collection, 
  addDoc, 
  deleteDoc, import React, { useState, useEffect, useRef } from 'react';
import { initializeApp } from 'firebase/app';
import { 
  getAuth, 
  signInWithEmailAndPassword,
      ...
      <div className="bg-white w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="p-8 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
             <div className="p-4 bg-slate-50 rounded-3xl"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Operatore</p><p className="font-bold text-slate-700 text-sm">{report.userName}</p></div>
             <div className="p-4 bg-slate-50 rounded-3xl"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Ore</p><p className="font-bold text-blue-600 text-xl">{report.hours} h</p></div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
             <div className="p-4 bg-slate-50 rounded-3xl border"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Veicolo</p><div className="flex items-center gap-2 text-slate-700 font-bold text-[10px] truncate"><Truck size={14}/> {report.vehicleName}</div></div>
          ...
          {report.equipmentUsed && (
            <div className="p-4 bg-blue-50/50 rounded-3xl border border-blue-100"><p className="text-[9px] font-black text-blue-400 uppercase mb-1">Attrezzatura</p><div className="flex items-center gap-2 text-blue-700 font-bold text-[10px]"><Construction size={14}/> {report.equipmentUsed}</div></div>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-50 p-5 rounded-3xl border text-center"><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Status Fase</p><p className="text-xs font-bold truncate uppercase">{next ? next.name : 'Fine'}</p></div>
      </div>
    </div>
  );
}
...
function SiteReportsList({ taskId, isMaster, userData, isAdminFull }) {
  ...
  return (
    <div className="space-y-4">
      {reports.length === 0 && <p className="text-center py-20 text-slate-300 font-black uppercase text-[10px]">Nessun rapporto storico</p>}
      {reports.map(r => (
...
function SiteAccounting({ taskId }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 animate-in slide-in-from-top-4">
      ...
      <div className="bg-slate-900 p-6 rounded-3xl shadow-xl text-center text-white"><p className="text-[9px] text-slate-400 uppercase font-black tracking-widest mb-1">Totale Costi</p><p className="text-2xl font-bold">€ {(totals.materials + totals.labor).toFixed(0)}</p></div>
    </div>
  );
}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in duration-500">
      ...
      <div className={`col-span-1 ${targetIdentifier === currentUser.uid ? 'md:col-span-2' : 'md:col-span-3'} space-y-3`}>
        {leaves.map(req => {
            const statusClass = req.status === 'approved' ? 'bg-green-100 text-green-700' : (req.status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700');
            return (
              <div key={req.id} className="bg-white p-6 rounded-[32px] border shadow-sm flex justify-between items-center border-slate-100">
                <div><div className="flex items-center gap-3"><span className={`px-3 py-1 rounded-lg text-[9px] font-black tracking-widest ${statusClass}`}>{req.status.toUpperCase()}</span><h4 className="font-bold text-slate-800 uppercase tracking-tighter">{req.type}</h4></div><p className="text-sm text-slate-500 mt-2 font-bold">{req.start} — {req.end}</p></div>
      ...
      {showForm && (
        <form onSubmit={add} className="bg-white p-10 rounded-[40px] border shadow-2xl space-y-5 animate-in slide-in-from-top-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input placeholder="Titolo" className="bg-slate-50 border-none rounded-2xl p-5 outline-none font-bold text-lg" onChange={e=>setForm({...form, title: e.target.value})} required />
            <input placeholder="Cliente" className="bg-slate-50 border-none rounded-2xl p-5 outline-none font-bold text-lg" onChange={e=>setForm({...form, client: e.target.value})} required />
          </div>
          ...
          <div className="flex gap-4 pt-4"><button type="submit" className="bg-blue-600 text-white px-12 py-4 rounded-3xl font-black text-xs uppercase tracking-widest shadow-xl">Salva</button><button type="button" onClick={()=>setShowForm(false)} className="text-slate-400 font-bold uppercase text-xs px-6">Annulla</button></div>
        </form>
      )}
