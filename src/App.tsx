...
// --- COMPONENTI INTERNI (DEFINITI PRIMA DELL'USO) ---
  ...
  return (
    <div className="bg-white border rounded-[32px] overflow-hidden shadow-sm overflow-x-auto">
      <table className="w-full text-left text-xs">
        ...
        <tbody className="divide-y">{logs.map(l=>(<tr key={l.id} className="hover:bg-slate-50"><td className="p-5 font-bold uppercase text-[10px]">{l.userName}</td><td className="p-5 text-[10px]">{l.action}</td><td className="p-5 font-mono text-blue-500 uppercase text-[9px]">{l.location}</td><td className="p-5 text-[9px] text-slate-400">{l.createdAt?.seconds ? new Date(l.createdAt.seconds * 1000).toLocaleString() : '-'}</td></tr>))}</tbody>
      </table>
    </div>
  );
}
...
function LeaveRequestsPanel({ currentUser, targetIdentifier, isMaster, isAdmin, userData }) {
  ...
  const [newRequest, setNewRequest] = useState({ start: '', end: '', type: 'Ferie' });
  ...
  const requestLeave = async (e) => {
    ...
    setNewRequest({ start: '', end: '', type: 'Ferie' });
    await logOperation(userData, "Richiesta Assenza", `${newRequest.type}`);
  };
        ...
        {leaves.map(req => (
            <div key={req.id} className="bg-white p-6 rounded-[32px] border shadow-sm flex justify-between items-center transition-all border-slate-100">
              ...
              {isAdmin && targetIdentifier !== currentUser.uid && req.status === 'pending' && (
                <div className="flex gap-2"><button onClick={() => handleStatus(req.id, 'approved')} className="bg-green-600 text-white px-5 py-2 rounded-xl text-xs font-black uppercase tracking-widest shadow-md">Ok</button><button onClick={() => handleStatus(req.id, 'rejected')} className="bg-red-50 text-red-600 px-5 py-2 rounded-xl text-xs font-black uppercase border border-red-100">No</button></div>
              )}
            </div>
          ))}
...
function SiteOverview({ task, isMaster, isAdmin, userData }) {
  ...
  const handleToggleStatus = async () => {
    ...
    if (newStatus && !window.confirm("Sei sicuro?")) return;
    if (!newStatus && !isAdmin) return;
    try {
      await updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'tasks', task.id), { completed: newStatus });
      await logOperation(userData, newStatus ? "Chiusura" : "Riapertura", task.title);
    ...
    <div className="space-y-6">
      <div className={`p-6 rounded-[32px] border flex flex-col sm:flex-row justify-between items-center gap-4 ${task.completed ? 'bg-slate-900 border-slate-800 text-white shadow-xl' : 'bg-white border-slate-200'}`}>
        <div className="flex items-center gap-4">
          ...
          <div><h4 className="font-black uppercase tracking-tighter">Stato Operativo</h4><p className={`text-xs font-bold ${task.completed ? 'text-slate-400' : 'text-blue-500'}`}>{task.completed ? 'CHIUSO' : 'APERTO'}</p></div>
        </div>
        {isMaster && (
          <div className="flex gap-2">
            {!task.completed ? <button onClick={handleToggleStatus} className="bg-red-600 text-white px-6 py-2.5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg">Chiudi</button> : isAdmin && <button onClick={handleToggleStatus} className="bg-blue-600 text-white px-6 py-2.5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg">Riapri</button>}
          </div>
        )}
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-3xl border shadow-sm flex flex-col justify-between"><div><p className="text-[9px] font-black text-slate-400 uppercase mb-1">Località</p><p className="text-sm font-black text-slate-600 uppercase">{task.isTrasfertaSite ? 'Fuori Sede' : 'Locale'}</p></div>{isAdmin && !task.completed && <button onClick={toggleTravelSite} className="text-[9px] text-blue-500 font-bold self-end">CAMBIA</button>}</div>
      </div>
    </div>
  );
}
...
function SiteDocuments({ taskId, isAdmin, userData, isClosed }) {
  ...
  const handleUpload = async (e) => {
    ...
    const reader = new FileReader(); reader.onloadend = async () => { await addDoc(collection(db, 'artifacts', APP_ID, 'public', 'data', 'documents'), { taskId, name: file.name, data: reader.result, uploadedBy: userData.name, createdAt: serverTimestamp() }); setUploading(false); }; reader.readAsDataURL(file);
  };
  return (
    <div className="space-y-4">
...
function SiteSchedule({ task, isAdmin }) {
  ...
  const addPhase = async (e) => {
    e.preventDefault(); if(!isAdmin || !newPhase.name || task.completed) return;
    const updated = [...schedule, newPhase].sort((a,b) => new Date(a.start) - new Date(b.start));
    await updateDoc(doc(db, 'artifacts', APP_ID, 'public', 'data', 'tasks', task.id), { schedule: updated }); setNewPhase({ name: '', start: '', end: '' });
  };
  return (
    <div className="space-y-4">
    ...
    <div className="space-y-8">
      ...
      {isFormOpen && (
        <form onSubmit={addTask} className="bg-white p-10 rounded-[40px] border shadow-2xl space-y-5 animate-in slide-in-from-top-4">
          ...
          <div className="flex gap-4 pt-4"><button type="submit" className="bg-blue-600 text-white px-12 py-4 rounded-3xl font-black text-xs uppercase tracking-widest shadow-xl">Salva Dati</button><button type="button" onClick={()=>setIsFormOpen(false)} className="text-slate-400 font-bold uppercase text-xs">Annulla</button></div>
        </form>
      )}
    ...
    <div className="space-y-8 max-w-2xl mx-auto">
      <form onSubmit={submit} className="bg-white p-10 rounded-[48px] border shadow-xl space-y-6 animate-in slide-in-from-bottom-4">
        ...
        <div className="border border-dashed rounded-[32px] p-6 bg-slate-50 text-center cursor-crosshair">
           <canvas ref={canvasRef} width={300} height={100} className="w-full h-32 bg-white rounded-3xl border shadow-inner touch-none" onMouseDown={(e) => { const ctx = e.target.getContext('2d'); ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(e.nativeEvent.offsetX, e.nativeEvent.offsetY); }} onMouseMove={(e) => { if(e.buttons !== 1) return; const ctx = e.target.getContext('2d'); ctx.lineTo(e.nativeEvent.offsetX, e.nativeEvent.offsetY); ctx.stroke(); }} />
        </div>
        <button type="submit" className="w-full bg-blue-600 text-white py-5 rounded-[28px] font-black uppercase tracking-widest text-sm shadow-xl">Invia Report</button>
      </form>
      <div className="space-y-4">
        {reports.map(r => (
          <div key={r.id} className="p-6 bg-white border rounded-[36px] flex justify-between items-center shadow-sm border-slate-100">
            <div className="flex-1 pr-4"><p className="font-bold text-slate-800 line-clamp-1 uppercase tracking-tight">{r.desc}</p><p className="text-[9px] text-slate-400 mt-2 uppercase font-black tracking-widest">{r.userName} • {r.taskTitle} • {r.hours} H • {r.vehicleName} {r.isTrasferta ? '• TRASFERTA' : ''}</p></div>
...
function VehiclesView({ isAdmin }) {
  ...
  const add = async (e) => { e.preventDefault(); if(!isAdmin) return; await addDoc(collection(db, 'artifacts', APP_ID, 'public', 'data', 'vehicles'), { ...newVehicle, assignedTo: 'Libero', createdAt: serverTimestamp() }); setNewVehicle({ name: '', plate: '', type: 'Furgone', insuranceDate: '', taxDate: '', inspectionDate: '' }); };
  const getStatus = (date) => { if(!date) return { label: '-', class: 'text-slate-300' }; const diff = (new Date(date) - new Date()) / (1000 * 60 * 60 * 24); if(diff < 0) return { label: 'SCADUTO', class: 'text-red-600 font-black' }; if(diff < 30) return { label: `${Math.ceil(diff)} gg`, class: 'text-orange-500 font-bold' }; return { label: new Date(date).toLocaleDateString(), class: 'text-slate-600 font-medium' }; };
  return (
    <div className="space-y-8">
      {isAdmin && (
        <form onSubmit={add} className="bg-white p-10 rounded-[48px] border shadow-lg space-y-6">
          <h3 className="text-2xl font-black text-slate-800 uppercase tracking-tighter flex items-center gap-4"><Truck size={28} className="text-blue-600"/> Censimento Mezzo</h3>
          ...
          <button type="submit" className="w-full bg-blue-600 text-white py-5 rounded-3xl font-black uppercase tracking-widest text-sm shadow-lg">Registra</button>
        </form>
      )}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {vehicles.map(v => {
          ...
          return (
            <div key={v.id} className="bg-white p-8 rounded-[40px] border shadow-sm relative group hover:shadow-lg transition-all border-slate-100">
              ...
              <div className="space-y-3 mt-6 border-t pt-6 border-slate-50">
                 <div className="flex justify-between text-xs font-bold uppercase tracking-widest"><span className="text-slate-400">Assic:</span><span className={ins.class}>{ins.label}</span></div>
                 <div className="flex justify-between text-xs font-bold uppercase tracking-widest"><span className="text-slate-400">Bollo:</span><span className={bol.class}>{bol.label}</span></div>
                 <div className="flex justify-between text-xs font-bold uppercase tracking-widest"><span className="text-slate-400">Revisione:</span><span className={rev.class}>{rev.label}</span></div>
              </div>
